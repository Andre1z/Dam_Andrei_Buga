...
def main():
    ...
    style = Style(theme='superhero')  # Cambia 'superhero' por el tema que prefieras

    # Crear un estilo personalizado para los botones
    style.configure('Custom.TButton', background='#4CAF50', foreground='white', font=('Helvetica', 10, 'bold'))
    style.map('Custom.TButton', background=[('active', '#45a049')])  # Cambiar color al hacer clic
    ...
    carpeta_button = ttk.Button(frame, text="Seleccionar Carpeta", command=seleccionar_carpeta, style='Custom.TButton')
    ...
    archivo_button = ttk.Button(frame, text="Seleccionar Archivo", command=seleccionar_archivo, style='Custom.TButton')
    ...
    estado_label = ttk.Label(frame, textvariable=estado_var, bootstyle="warning")  # Cambiar estilo del texto
